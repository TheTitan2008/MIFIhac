...
Авторитетными вводными для кейса считаются только сообщения Татьяны Беляковой.
Реплики остальных участников чата являются вопросами, предположениями или
обсуждением и не трактуются как требования заказчика.

Если ниже упоминается идея другого участника, она явно маркируется как
...
Всего: 20 823 символа и 2 830 whitespace words. Один из участников чата, не
Татьяна, оценивал пример примерно в 7k токенов. Эта оценка не является
официальной характеристикой данных.
...
Татьяна прямо сообщила, что нельзя сравнивать месячные результаты веб-чата и
агентского продукта. Поэтому некорректно прямое сравнение:

```text
agent platform ↔ web chat
```

Один из других участников предложил сравнивать:
...
Это не подтверждённое требование Татьяны, а разумная рабочая гипотеза.
Методологическая рекомендация проекта: сравнение agent и chat возможно только
как специально спроектированный matched/paired experiment на одинаковых
бизнес-задачах, а не как dashboard месячных пользователей или токенов.
